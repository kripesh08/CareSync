...
public class TokenController {
    ...
    public ResponseEntity<?> createRazorpayOrder(@PathVariable Long tokenId, HttpServletRequest httpRequest) {
        try {
            ...
            return ResponseEntity.badRequest()
                    .body(new ErrorResponse("Failed to create payment order: " + e.getMessage()));
        }
    }
    ...
    @PreAuthorize("hasRole('PATIENT')")
    public ResponseEntity<?> processPayment(@PathVariable Long tokenId, @RequestBody PaymentRequest request,
    ...
    public ResponseEntity<?> getHospitalTokensToday(@RequestParam(required = false) String date,
    ...
    @PreAuthorize("hasRole('HOSPITAL')")
    public ResponseEntity<?> getCompletedTokens(@RequestParam(required = false) String date,
            HttpServletRequest request) {
        try {
            User hospitalUser = getCurrentUser(request);
            LocalDate queryDate = date != null ? LocalDate.parse(date) : LocalDate.now();
            List<QueueToken> tokens = tokenService.getHospitalTokensByStatus(hospitalUser, queryDate,
                    QueueToken.TokenStatus.COMPLETED);
            return ResponseEntity.ok(tokens);
        } catch (RuntimeException e) {
            return ResponseEntity.badRequest().body(new ErrorResponse(e.getMessage()));
        }
    ...
    @PreAuthorize("hasRole('HOSPITAL')")
    public ResponseEntity<?> getTokenStatistics(@RequestParam(required = false) String date,
            HttpServletRequest request) {
        try {
            User hospitalUser = getCurrentUser(request);
            LocalDate queryDate = date != null ? LocalDate.parse(date) : LocalDate.now();
