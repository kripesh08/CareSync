...
public class TokenService {
        ...
        if (queue.getOperatingDays() != null && !queue.getOperatingDays().isEmpty()) {
            ...
            if (!queue.getOperatingDays().contains(dayOfWeek)) {
                throw new RuntimeException(
                        "Queue is not available on " + dayOfWeek.charAt(0) + dayOfWeek.substring(1).toLowerCase());
            }
        }

        // Check capacity — only count paid tokens
        long currentTokens = tokenRepository.countByQueueAndTokenDateAndPaymentStatus(queue, tokenDate,
                QueueToken.PaymentStatus.COMPLETED);
        if (currentTokens >= queue.getMaxCapacity()) {
            throw new RuntimeException("Queue is full for this date");
        }

        // Check if patient already has a pending or active token for this queue and
        // date
    ...
    public List<QueueToken> getHospitalTokensByStatus(User hospitalUser, LocalDate date,
            QueueToken.TokenStatus status) {
        Hospital hospital = getHospitalByUser(hospitalUser);
        return tokenRepository.findByHospitalIdAndTokenDateAndTokenStatusOrderByTokenNumberAsc(hospital.getHospitalId(),
                date, status);
    }
    ...
    public QueueToken callToken(User hospitalUser, Long tokenId) {
        ...
        if (!inProgress.isEmpty()) {
            throw new RuntimeException("Token #" + inProgress.get(0).getTokenNumber()
                    + " is already in progress. Complete it before calling the next token.");
        }
    ...
    public Map<String, Object> getQueueStatus(Long tokenId, User patient) {
        ...
        // Count active tokens ahead (WAITING or IN_PROGRESS tokens with lower number)
        if (token.getTokenNumber() != null) {
            long tokensAhead = tokenRepository.countActiveAhead(
                    token.getQueue(), token.getTokenDate(), token.getTokenNumber());
            status.put("tokensAhead", tokensAhead);
        } else {
            // For pending tokens, all currently active tokens (WAITING/IN_PROGRESS) are effectively ahead
            long tokensAhead = tokenRepository.countAllActive(token.getQueue(), token.getTokenDate());
            status.put("tokensAhead", tokensAhead);
        }
        ...
        // Get AI-based prediction for more accurate avgTimePerPatient
        double avgTime = token.getQueue().getEstimatedTimePerPatient() != null ? token.getQueue().getEstimatedTimePerPatient() : 15.0;
        try {
            // We use a dummy token number (like totalTokens + 1) to get the current predicted average
            var prediction = predictionService.predictWaitingTime(token.getQueue().getQueueId(), token.getTokenDate(), (int)totalTokens + 1);
            avgTime = prediction.getAverageTimePerToken();
        } catch (Exception e) {
            // Fallback to static queue setting
        }
        
        // Add queue timing info for client-side estimated arrival calculation
        status.put("avgTimePerPatient", avgTime);
        status.put("queueStartTime",
                token.getQueue().getStartTime() != null ? token.getQueue().getStartTime().toString() : null);
    ...
    private Hospital getHospitalByUser(User user) {
        return hospitalRepository.findByUser_UserId(user.getUserId())
                .orElseThrow(() -> new RuntimeException(
                        "Hospital profile not found for user ID: " + user.getUserId() + " (" + user.getEmail() + ")"));
    }
    ...
    public static class TokenStatistics {
        ...
        public long getInProgressTokens() {
